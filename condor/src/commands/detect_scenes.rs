...
use av1an_core::condor::core::input::Input;
...
pub fn detect_scenes_handler(
    ...
    let mut input_instance = Input::from_data(&config.condor.input)?;
    let clip_info = input_instance.clip_info()?;
    let fps = *clip_info.frame_rate.numer() as f64 / *clip_info.frame_rate.denom() as f64;

    let previous_method = config.condor.processor_config.scene_detection.method;
    ...
    if let Some(new_method) = new_method {
        config.condor.processor_config.scene_detection.method = new_method;
    }
    config
        .condor
        .processor_config
