...
fn build_decoder(
    ...
    let decoder = if input.is_vapoursynth_script() {
        // Use VapoursynthDecoder for all VapourSynth scripts (both user-provided and
        // generated)
        ...
        let mut vs_decoder = if input.is_vapoursynth() {
            ...
            // Generated VapourSynth script
            VapoursynthDecoder::from_file(input.as_script_path())?
        };

        vs_decoder.set_variables(input.as_vspipe_args_hashmap()?)?;

        // Apply downscaling if requested and needed
        if let Some(downscale_height) = sc_downscale_height {
            if downscale_height < (input_height as usize) {
